...
describe('utils.js', () => {
  describe('createUrlHash', () => {
    ...
    test('should create different hashes for different URLs', () => {
      ...
      assert.notEqual(hash1, hash2, 'Different URLs should produce different hashes');
    });
  ...
  describe('readUrlsFromDirectory', () => {
    test('should read URLs from txt files', async () => {
      ...
      await createTestFile('urls/hash1.txt', 'https://www.youtube.com/watch?v=video1');
      await createTestFile('urls/hash2.txt', 'https://www.youtube.com/watch?v=video2');
    ...
    test('should return empty array for non-existent directory', async () => {
      const logger = createMockLogger();
      const urls = await readUrlsFromDirectory('/non/existent/path', 'test', logger);
  ...
  describe('getDownloadedFiles', () => {
    test('should return some files from actual downloads directory', async () => {
      ...
      for (const group of groupedFiles) {
        ...
        if (group.video) {
          ...
          assert.ok('isSubtitle' in group.video, 'Video should have isSubtitle');
          assert.equal(group.video.isVideo, true, 'Video should be marked as video');
          assert.equal(group.video.isSubtitle, false, 'Video should not be marked as subtitle');
        }
        ...
        for (const subtitle of group.subtitles) {
          ...
          assert.ok('isSubtitle' in subtitle, 'Subtitle should have isSubtitle');
          assert.equal(subtitle.isVideo, false, 'Subtitle should not be marked as video');
          assert.equal(subtitle.isSubtitle, true, 'Subtitle should be marked as subtitle');
        }
      }
    });
    ...
    test('should detect file types correctly with regex patterns', () => {
      ...
      for (const ext of videoExtensions) {
        ...
        assert.ok(/\.(mkv|mp4|webm|avi|mov)$/i.test(filename), `${ext} should match video pattern`);
        assert.ok(!/\.(srt|vtt)$/i.test(filename), `${ext} should not match subtitle pattern`);
      }
      
      for (const ext of subtitleExtensions) {
        const filename = `test.${ext}`;
        assert.ok(/\.(srt|vtt)$/i.test(filename), `${ext} should match subtitle pattern`);
        assert.ok(!/\.(mkv|mp4|webm|avi|mov)$/i.test(filename), `${ext} should not match video pattern`);
      }
    });
    ...
    test('should test baseName extraction logic', () => {
      const testCases = [
        ...
        { filename: 'long-filename-with-dashes.webm', expectedBaseName: 'long-filename-with-dashes' },
        { filename: 'file.with.dots.avi', expectedBaseName: 'file.with.dots' },
        { filename: 'UPPERCASE.MP4', expectedBaseName: 'UPPERCASE' },
      ];
      
      for (const testCase of testCases) {
        const baseName = testCase.filename.replace(/\.(mkv|mp4|webm|avi|mov|srt|vtt)$/i, '');
        assert.equal(baseName, testCase.expectedBaseName, 
