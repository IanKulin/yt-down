...
describe('api.js', () => {
  describe('GET /api/state', () => {
    ...
    test('should test state object structure for API response', () => {
      ...
      // Verify structure
      assert.ok(Array.isArray(expectedStructure.queued), 'queued should be array');
      assert.ok(Array.isArray(expectedStructure.active), 'active should be array');
      assert.ok(Array.isArray(expectedStructure.finished), 'finished should be array');
      assert.ok(typeof expectedStructure.counts === 'object', 'counts should be object');
      assert.ok(typeof expectedStructure.processor === 'object', 'processor should be object');
      assert.ok(typeof expectedStructure.timestamp === 'string', 'timestamp should be string');
      ...
      assert.ok('isProcessing' in processor, 'processor should have isProcessing');
      assert.ok('activeDownloads' in processor, 'processor should have activeDownloads');
      assert.ok('maxConcurrent' in processor, 'processor should have maxConcurrent');
      assert.ok('pollInterval' in processor, 'processor should have pollInterval');
    });
    ...
    test('should test count calculations', () => {
      const testCases = [
        ...
        {
          queued: 5, active: 2, finished: 10,
          expectedTotal: 17
        },
        {
          queued: 100, active: 0, finished: 50,
          expectedTotal: 150
        },
      ];
      ...
      for (const testCase of testCases) {
        ...
        assert.equal(total, testCase.expectedTotal, 
    ...
    test('should test error response structure', () => {
      ...
      assert.ok('error' in errorResponse, 'Error response should have error field');
      assert.ok('timestamp' in errorResponse, 'Error response should have timestamp field');
      assert.equal(typeof errorResponse.error, 'string', 'Error should be string');
      assert.equal(typeof errorResponse.timestamp, 'string', 'Timestamp should be string');
    });
