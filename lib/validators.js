...
const validateUrl = (url) => {
  ...
  if (trimmedUrl.length > 2048) {
    throw new ValidationError('URL must be 2048 characters or less');
  }
...
const validateFilename = (filename) => {
  ...
  // Check for .. only as complete path components (not just anywhere in filename)
  const pathComponents = trimmedFilename.split(/[/\\]/);
...
const validateSettings = (settings) => {
  ...
  if (settings.rateLimit !== undefined) {
    ...
    const isValidNumber = typeof settings.rateLimit === 'number' &&
      settings.rateLimit >= 0;
    const isValidString = typeof settings.rateLimit === 'string' &&
...
export { validateFilename, validateSettings, validateUrl };
