...
export const DOWNLOADS_ACTIVE_DIR = path.join(__dirname, '..', 'data', 'downloads', 'active');
export const DOWNLOADS_FINISHED_DIR = path.join(__dirname, '..', 'data', 'downloads', 'finished');
...
export async function cleanupActiveDownloads(logger) {
  try {
    ...
    for (const file of urlFiles) {
      if (file === '.DS_Store' || !file.endsWith('.txt')) continue;
      ...
      try {
        ...
        logger.info(`Moved interrupted download back to queue: ${file}`);
      } catch (error) {
