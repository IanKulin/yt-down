...
import { crypto as denoStdCrypto } from '@std/crypto';
...
const DEFAULT_BASE_DIR = join(__dirname, '..');
...
export class Job {
  ...
  generateId(url) {
    ...
    if (!url || typeof url !== 'string') {
      const hashBuffer = denoStdCrypto.subtle.digestSync(
        'SHA-256',
        encoder.encode(''),
      );
  ...
  getFilePath(baseDir = DEFAULT_BASE_DIR) {
    const directory = join(baseDir, 'data', 'jobs', this.state);
    return join(directory, this.getFilename());
  }
  ...
  /**
   * Convert job to API format for templates and API responses
   */
  toApiFormat(options = {}) {
    const base = {
      hash: this.id,
      ...
      timestamp: this.timestamp,
      sortOrder: this.sortOrder,
      filesize: this.metadata?.filesize || null,
    };

    if (options.includeFilePath) {
      base.filePath = this.getFilePath(options.baseDir);
    }
...
export class JobManager {
  constructor(options = {}) {
    ...
    this.jobDirectories = {
      [JobState.QUEUED]: join(this.baseDir, 'data', 'jobs', 'queued'),
      [JobState.ACTIVE]: join(this.baseDir, 'data', 'jobs', 'active'),
      [JobState.FAILED]: join(this.baseDir, 'data', 'jobs', 'failed'),
    };
  }
  ...
  async createJob(url, options = {}) {
    ...
    // Check if job already exists in active queues only
    const existingJob = await this.getJobInActiveQueues(job.id);
    if (existingJob) {
  ...
  /**
   * Get a job by ID from active queues only (queued/active)
   * Used for duplicate prevention
   */
  async getJobInActiveQueues(jobId) {
    for (const state of [JobState.QUEUED, JobState.ACTIVE]) {
      const directory = this.jobDirectories[state];
      if (!directory) continue;

      const filePath = join(directory, `${jobId}.json`);
      try {
  ...
  /**
   * Get a job by ID from all directories (including failed and finished for backward compatibility)
   */
  async getJob(jobId) {
    // Check active queues first
    const activeJob = await this.getJobInActiveQueues(jobId);
    if (activeJob) {
      return activeJob;
    }

    // Check failed directory
    const failedDir = this.jobDirectories[JobState.FAILED];
    const failedFilePath = join(failedDir, `${jobId}.json`);
    try {
      await Deno.stat(failedFilePath);
      return await this.readJobFile(failedFilePath);
    } catch {
      // File doesn't exist in failed directory
    }

    // Check finished directory for backward compatibility during migration
    const finishedDir = join(this.baseDir, 'data', 'jobs', 'finished');
    const finishedFilePath = join(finishedDir, `${jobId}.json`);
    try {
      await Deno.stat(finishedFilePath);
      return await this.readJobFile(finishedFilePath);
    } catch {
      // File doesn't exist in finished directory
    }
  ...
  async getJobsByState(state) {
    ...
    try {
      await this.ensureDirectoryExists(directory);
      const dirEntries = await Array.fromAsync(Deno.readDir(directory));
      ...
      for (const entry of dirEntries) {
        if (entry.name.endsWith('.json')) {
          ...
          try {
            ...
            this.logger?.error(
              `Error parsing job file ${entry.name}:`,
              parseError,
            );
   ...
   */
  async getFailedJobs() {
    return await this.getJobsByState(JobState.FAILED);
  }
  ...
  async moveJob(jobId, newState) {
    ...
    try {
      await Deno.remove(currentJob.getFilePath(this.baseDir));
    } catch (error) {
  ...
  async deleteJob(jobId) {
    ...
    await Deno.remove(job.getFilePath(this.baseDir));
  ...
  async handleJobFailure(jobId, error) {
    ...
    if (job.retryCount >= this.maxRetries) {
      this.logger?.warn(
        `Max retries (${this.maxRetries}) reached for job ${jobId}, moving to failed directory`,
      );
      await this.moveJobToFailed(jobId, error);
      return null;
    }
  ...
  /**
   * Move job to failed directory with failure metadata
   */
  async moveJobToFailed(jobId, error) {
    const job = await this.getJob(jobId);
    if (!job) {
      throw new Error(`Job not found: ${jobId}`);
    }
    ...
    // Create failed job with updated metadata
    const failedJob = job.clone(JobState.FAILED);
    failedJob.metadata = job.metadata;

    // Write to failed directory
    const failedDirectory = this.jobDirectories[JobState.FAILED];
    await this.ensureDirectoryExists(failedDirectory);
    await this.writeJobFile(failedJob.getFilePath(this.baseDir), failedJob);
    ...
    try {
      ...
      this.logger?.warn(
        `Failed to remove old job file: ${unlinkError.message}`,
      );
    }

    this.logger?.info(
      `Moved job ${jobId} to failed directory: ${
  ...
  async retryFailedJob(jobId) {
    ...
    // Reset retry count and remove failure metadata
    job.retryCount = 0;
    if (job.metadata) {
      delete job.metadata.lastError;
      delete job.metadata.failedAt;
    }

    // Move job back to queued
    const queuedJob = await this.moveJob(jobId, JobState.QUEUED);

    this.logger?.info(`Retried failed job ${jobId}`);
    return queuedJob;
  }
   ...
   */
  async getJobStats() {
    const [queued, active, failed] = await Promise.all([
      this.getQueuedJobs(),
      this.getActiveJobs(),
      this.getFailedJobs(),
    ]);
    ...
    return {
      ...
      total: queued.length + active.length + failed.length,
    };
  }
}
