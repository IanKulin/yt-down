...
export class Job {
  ...
  toApiFormat(options = {}) {
    const base = {
      ...
      failedAt: this.metadata?.failedAt || null,
    };
...
export class JobManager {
  constructor(options = {}) {
    ...
    this.jobDirectories = {
      ...
      [JobState.FAILED]: path.join(this.baseDir, 'data', 'jobs', 'failed'),
    };
  }
  ...
  async getJob(jobId) {
    ...
    const failedFilePath = path.join(failedDir, `${jobId}.json`);
    try {
      await fs.access(failedFilePath);
    ...
    const finishedDir = path.join(this.baseDir, 'data', 'jobs', 'finished');
    const finishedFilePath = path.join(finishedDir, `${jobId}.json`);
    try {
      await fs.access(finishedFilePath);
  ...
  async moveJobToFailed(jobId, error) {
    ...
    this.logger?.info(
      `Moved job ${jobId} to failed directory: ${error?.message || 'Unknown error'}`
    );
    return failedJob;
  }
