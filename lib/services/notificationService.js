...
export class NotificationService {
  constructor({ logger, baseDir, broadcastChange }) {
    this.logger = logger;
    this.baseDir = baseDir || join(__dirname, '..', '..');
    this.notificationsFile = join(this.baseDir, 'data', 'notifications.json');
  ...
  async ensureNotificationsDirectory() {
    const dir = dirname(this.notificationsFile);
    await ensureDir(dir);
  }
  ...
  async getNotifications() {
    try {
      const data = await Deno.readTextFile(this.notificationsFile);
