...
export class NotificationService {
  constructor({ logger, baseDir }) {
    ...
    this.notificationsFile = path.join(this.baseDir, 'data', 'notifications.json');
  }
  ...
  async getNotifications() {
    try {
      ...
      if (error.code === 'ENOENT') {
        ...
        this.logger?.debug('No notifications file found, returning empty array');
        return [];
      }
      
      this.logger?.warn('Error reading notifications file, returning empty array:', error);
      return [];
    }
  }
  ...
  async dismissNotification(notificationId) {
    ...
    try {
      ...
      if (removed > 0) {
        ...
        this.logger?.warn(`Notification not found for dismissal: ${notificationId}`);
        return {
          success: true,
