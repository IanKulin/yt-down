// No imports needed - using Deno.Command
...
export class VersionService {
  ...
  async detectYtDlpVersion() {
    try {
      ...
      if (code !== 0) {
        this.logger?.debug('yt-dlp command exited with code:', code);
        return null;
      }
  ...
  async detectFfmpegVersion() {
    try {
      ...
      if (code !== 0) {
        this.logger?.debug('ffmpeg command exited with code:', code);
        return null;
      }

      // ffmpeg outputs version info to stdout
