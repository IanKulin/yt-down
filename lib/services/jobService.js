...
export class JobService {
  ...
  async getJobStatistics() {
    ...
    return {
      counts: {
        ...
        total: jobs.queued.length + jobs.active.length + jobs.failed.length,
      },
    ...
    try {
      ...
      if (this.notificationService) {
        await this.notificationService.addNotification(
          'job_added',
          'Download job added to queue successfully',
          { url: validatedUrl }
        );
      }
    ...
    try {
      ...
      if (job.state === 'active') {
        ...
        if (this.notificationService) {
          await this.notificationService.addNotification(
            'job_deleted',
            'Download job deleted from queue successfully',
            { hash: trimmedHash }
          );
        }
   ...
   */
  async getFailedJobs() {
    const jobs = await this.jobManager.getFailedJobs();
    return jobs.map((job) => job.toApiFormat());
  }

  /**
   * Retry a failed job
   * @param {string} jobHash - The job hash to retry
   * @returns {Object} Success result with message
   */
  async retryFailedJob(jobHash) {
    if (!jobHash || !jobHash.trim()) {
      throw new ValidationError('Invalid hash provided');
    }
    ...
    try {
      // Get the job to verify it exists and is in failed state
      const job = await this.jobManager.getJob(trimmedHash);
      if (!job) {
      ...
      if (job.state !== 'failed') {
        throw new ValidationError('Job is not in failed state');
      }

      // Retry the job using JobManager
      await this.jobManager.retryFailedJob(trimmedHash);
      ...
      this.logger?.info(`Retried failed job: ${trimmedHash}`);

      // Send notification directly
      if (this.notificationService) {
        await this.notificationService.addNotification(
          'job_retried',
          'Failed job retried and moved back to queue',
          { hash: trimmedHash }
        );
      }
   ...
   * @param {string} jobHash - The job hash to delete
   * @returns {Object} Success result
   */
  async deleteFailedJob(jobHash) {
    if (!jobHash || !jobHash.trim()) {
      throw new ValidationError('Invalid hash provided');
    }
    ...
    this.logger?.info(`Deleted failed job: ${trimmedHash}`);
    return { success: true };
  }
}
