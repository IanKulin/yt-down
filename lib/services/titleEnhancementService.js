...
import { isSbsUrl, extractMpxId, getSbsMetadata } from '../sbsResolver.js';
import { isBrollieUrl, getBrollieMetadata } from '../brollieResolver.js';
...
export class TitleEnhancementService {
  ...
  async enhanceJobMetadata(job) {
    ...
    try {
      ...
      let metadata;
      if (isSbsUrl(job.url)) {
        const mpxId = extractMpxId(job.url);
        const sbsMeta = await getSbsMetadata(mpxId);
        const title = sbsMeta.seriesTitle
          ? `${sbsMeta.seriesTitle} S${sbsMeta.seasonNumber} Ep${sbsMeta.episodeNumber}`
          : sbsMeta.title;
        metadata = { ...sbsMeta, title };
      } else if (isBrollieUrl(job.url)) {
        metadata = await getBrollieMetadata(job.url);
      } else {
        metadata = await this.extractVideoMetadata(
          job.url,
