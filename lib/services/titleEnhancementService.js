...
export class TitleEnhancementService {
    ...
    try {
      ...
      if (metadata) {
        ...
        if (
          ...
          this.logger?.info(
            `Enhanced job ${job.id} with metadata: ${
              metadata.title || 'filesize update'
            }`,
          );
        } else {
          this.logger?.info(
  ...
  extractVideoMetadata(url, timeout = 15000) {
    return new Promise((resolve) => {
      ...
      // Read stderr stream (discard output but consume the stream to prevent resource leak)
      (async () => {
        for await (const _chunk of ytDlp.stderr) {
