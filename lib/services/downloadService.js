...
import { DOWNLOADS_FINISHED_DIR, getDownloadedFiles, formatFileSize } from '../utils.js';
...
export class DownloadService {
  ...
  async validateFileAccess(filename) {
    ...
    if (!resolvedPath.startsWith(resolvedDownloadsDir)) {
      this.logger?.warn(`Access denied for file outside downloads directory: ${filename}`);
  ...
  async prepareFileDownload(filename) {
    const { filePath, validatedFilename } = await this.validateFileAccess(filename);
  ...
  async deleteFile(filename) {
    ...
    try {
      ...
      if (error.code === 'ENOENT') {
        this.logger?.warn(`File not found during deletion: ${validatedFilename}`);
        throw new NotFoundError('File not found');
      }
  ...
  async getFileStats(filename) {
    ...
    try {
      ...
      this.logger?.error(`Error getting file stats for ${validatedFilename}:`, error);
      throw error;
    }
  }
