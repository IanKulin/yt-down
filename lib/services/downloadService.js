...
export class DownloadService {
  ...
  async validateFileAccess(filename) {
    ...
    const filePath = join(DOWNLOADS_FINISHED_DIR, validatedFilename);

    // Security check: ensure the file is within the downloads directory
    const resolvedPath = resolve(filePath);
    const resolvedDownloadsDir = resolve(DOWNLOADS_FINISHED_DIR);
  ...
  async prepareFileDownload(filename) {
    const { filePath, validatedFilename } = await this.validateFileAccess(
      filename,
    );
    ...
    const encodedFilename = encodeURIComponent(validatedFilename);
    // deno-lint-ignore no-control-regex
    const hasNonAscii = /[^\x00-\x7F]/.test(validatedFilename);
