...
class QueueProcessor {
  ...
  async handleDownloadError(hash, url, error, metadata = {}) {
    ...
    try {
      ...
      if (job && job.retryCount < this.jobManager.maxRetries) {
        ...
        await this.cleanupJobPartialFiles({
          hash,
          url,
          ...metadata,
        });
  ...
  /**
   * Clean up partial files for a specific job before retry
   * @param {Object} job - Job object with id and progress information
   */
  async cleanupJobPartialFiles(job) {
    try {
      const activeFiles = await fs.readdir(this.downloadsActiveDir);
      const progress = this.downloadProgress.get(job.hash);

      // If we have a known filename from progress, try to clean up files that might match
      if (progress?.filename) {
        const baseFilename = path.parse(progress.filename).name;

        for (const file of activeFiles) {
          if (file === '.DS_Store') continue;

          // Check if file might be related to this job
          // This includes partial files, temp files, and files with similar names
          const shouldClean =
            file.includes(baseFilename) ||
            file.startsWith(job.hash) ||
            file.includes(job.hash.substring(0, 8)); // First 8 chars of hash
          ...
          if (shouldClean) {
            ...
            try {
              await fs.unlink(filePath);
              this.logger?.info(
                `Cleaned up partial file for job ${job.hash}: ${file}`
              );
            } catch (error) {
              this.logger?.warn(
                `Failed to clean up partial file ${file}:`,
                error
              );
            }
          }
        ...
        // Fallback: clean up recent files (similar to existing logic)
        const cutoffTime = Date.now() - this.CLEANUP_CUTOFF_MINUTES * 60 * 1000;
        ...
        for (const file of activeFiles) {
          ...
          try {
            ...
            if (stats.mtime.getTime() > cutoffTime) {
              await fs.unlink(filePath);
              this.logger?.info(
                `Cleaned up recent partial file for job ${job.hash}: ${file}`
              );
            }
          } catch (error) {
            this.logger?.warn(
      ...
      this.logger?.error(
        `Error cleaning up partial files for job ${job.hash}:`,
        error
      );
    }
  }
