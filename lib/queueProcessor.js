...
class QueueProcessor {
  constructor(options = {}) {
    ...
    this.queuedDir = path.join(this.baseDir, 'data', 'jobs', 'queued');
    this.activeDir = path.join(this.baseDir, 'data', 'jobs', 'active');
    this.finishedDir = path.join(this.baseDir, 'data', 'jobs', 'finished');
    this.downloadsActiveDir = path.join(
      this.baseDir,
      'data',
      'downloads',
      'active'
    );
    this.downloadsFinishedDir = path.join(
      this.baseDir,
      'data',
      'downloads',
      'finished'
        ...
        if (file.endsWith('.json')) {
          ...
          try {
            ...
            jobs.push({
              ...
              url: jobItem.url,
              title: jobItem.title,
              retryCount: jobItem.retryCount || 0,
              timestamp: jobItem.timestamp,
              sortOrder: jobItem.sortOrder || 0,
              filePath,
            });
          } catch (parseError) {
            this.logger?.error(`Error parsing JSON file ${file}:`, parseError);
  ...
  async startDownload(job) {
    const { hash, url, filePath, title, retryCount, timestamp, sortOrder } =
      job;

    try {
      // Move file to active directory
      const activeFilePath = path.join(this.activeDir, `${hash}.json`);
  ...
  async completeDownload(hash, url, activeFilePath, _metadata = {}) {
    try {
      // Read the active file to get updated metadata
      const data = await fs.readFile(activeFilePath, 'utf-8');
      const jobItem = JSON.parse(data);

      // Update with any title extracted during download
      const progress = this.downloadProgress.get(hash);
      if (progress?.filename && !jobItem.title) {
        jobItem.title = progress.filename;
      }

      // Move job tracking file to finished directory
      const finishedFilePath = path.join(this.finishedDir, `${hash}.json`);
      await this.ensureDirectoryExists(this.finishedDir);
      await fs.writeFile(finishedFilePath, JSON.stringify(jobItem, null, 2));
  ...
  async handleDownloadError(hash, url, activeFilePath, error, _metadata = {}) {
    this.logger?.error(`Download failed for ${url} (hash: ${hash}):`, error);

    try {
      // Read the active file to get current metadata
      const data = await fs.readFile(activeFilePath, 'utf-8');
      const jobItem = JSON.parse(data);

      // Increment retry count
      jobItem.retryCount = (jobItem.retryCount || 0) + 1;

      // Check if we should retry or give up
      const maxRetries = 3; // Could be configurable
      if (jobItem.retryCount >= maxRetries) {
        this.logger?.info(
          `Max retries (${maxRetries}) reached for ${url}, giving up`
        );
        await fs.unlink(activeFilePath);
        return;
      }
      ...
      const queuedFilePath = path.join(this.queuedDir, `${hash}.json`);
      await fs.writeFile(queuedFilePath, JSON.stringify(jobItem, null, 2));
      await fs.unlink(activeFilePath);
      this.logger?.info(
        `Moved failed download back to queue: ${url} (retry ${jobItem.retryCount}/${maxRetries})`
      );
    } catch (moveError) {
      this.logger?.error(
