...
import { getYtDlpArgs, loadSettings } from './settings.js';
import { isSbsUrl, resolveHlsUrl } from './sbsResolver.js';
import { isBrollieUrl, resolveStreamUrl } from './brollieResolver.js';
...
class QueueProcessor {
  constructor(options = {}) {
    ...
    this.activeDownloadHash = null; // Track the hash of the active download
    ...
    this.addNotification = null; // Notification callback
    this.logService = options.logService || null; // Stderr log service
    ...
    this._reconnectLoop = null; // Background reconnection loop

    // Constants
    this.PROGRESS_BROADCAST_THROTTLE = 1000; // Throttle progress broadcasts to once per second
    this.CONNECTIVITY_TIMEOUT = 3000; // 3-second DNS timeout
    this.RECONNECT_POLL_INTERVAL = 15000; // 15 seconds between connectivity retries
  }
  ...
  checkConnectivity() {
    // Use a TCP socket to a hardcoded external IP rather than DNS.
    // DNS would query Docker's embedded resolver (127.0.0.11) which is always
    // reachable locally and caches results — it cannot detect host network loss.
    // A TCP connect to 8.8.8.8:53 requires actual packet routing to the internet.
    return new Promise((resolve) => {
      const socket = new net.Socket();
      socket.setTimeout(this.CONNECTIVITY_TIMEOUT);
  ...
  pause(reason) {
    this.isPaused = true;
    this.pauseReason = reason || 'Unknown reason';
    this.logger?.warn(`Queue paused: ${this.pauseReason}`);
  ...
  resume() {
    ...
    this.processQueue().catch((error) => {
      this.logger?.error('Error processing queue after resume:', error);
    });
  }

  _startReconnectLoop() {
    if (this._reconnectLoop) return; // Already running
    this._reconnectLoop = true;

    const poll = async () => {
      if (!this.isPaused || !this._reconnectLoop) return;
      ...
      if (reachable) {
        ...
        if (this.addNotification) {
          await this.addNotification(
            'success',
            'Queue resumed: connectivity restored',
            {}
          );
        }
      } else {
        setTimeout(poll, this.RECONNECT_POLL_INTERVAL);
      }
    };
  ...
  async stop() {
    ...
    // Wait for active download to complete
    if (this.isDownloadActive && this.activeDownloadHash) {
      this.logger?.info(
        `Waiting for active download to complete: ${this.activeDownloadHash}`
      );
      // Wait for the active download to finish by checking the flag
      while (this.isDownloadActive) {
  ...
  async processQueue() {
    ...
    try {
      ...
      // Check connectivity whenever there is active or pending work
      if (this.isDownloadActive || queuedJobs.length > 0) {
        const reachable = await this.checkConnectivity();
        if (!reachable) {
          this.pause('No internet connectivity');
          if (this.addNotification) {
            await this.addNotification(
              'error',
              'Queue paused: no internet connectivity detected',
              {}
            );
          }
      ...
      // Start the first queued job (only one at a time)
      if (queuedJobs.length > 0) {
        await this.startDownload(queuedJobs[0]);
      }
    } catch (error) {
  ...
  async startDownload(job) {
    // prettier-ignore because you are wrong, wrong damnit
    const { hash, url, title, retryCount, timestamp, sortOrder, filesize } =
      job;

    try {
      // Move job to active state using JobManager
      const activeJob = await this.jobManager.moveJob(hash, JobState.ACTIVE);

      // Initialize progress tracking with any existing filesize metadata
      const progress = this.downloadProgress.get(hash) || {};
      if (filesize || activeJob.metadata?.filesize) {
        progress.filesize = filesize || activeJob.metadata.filesize;
      }
      this.downloadProgress.set(hash, progress);
      ...
      // Set download as active
      this.isDownloadActive = true;
      this.activeDownloadHash = hash;
  ...
  async downloadVideo(hash, url, metadata = {}) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        if (isSbsUrl(url)) {
          ...
          const { email, password } = settings.sbs ?? {};
          if (!email || !password) {
            throw new Error(
              'SBS credentials not configured — add them in Settings.'
            );
          }
          this.logger?.info(`Resolving SBS URL to HLS stream: ${url}`);
          downloadUrl = await resolveHlsUrl(url, email, password);
          this.logger?.debug(`Resolved HLS URL: ${downloadUrl}`);

          // Capture title so we can supply it explicitly to yt-dlp — a raw
          // HLS URL has no title metadata, so %(title)s falls back to the
          // stream filename (e.g. "master.mp4").
          resolvedTitle = metadata.title;
          if (!resolvedTitle) {
            const job = await this.jobManager.getJob(hash);
            resolvedTitle = job?.title;
          }
        } else if (isBrollieUrl(url)) {
          this.logger?.info(`Resolving Brollie URL to HLS stream: ${url}`);
          const brollieStream = await resolveStreamUrl(url);
          downloadUrl = brollieStream.url;
          this.logger?.debug(`Resolved HLS URL: ${downloadUrl}`);

          // Prefer the title from the stream API response; fall back to
          // whatever the title enhancement service may have already set.
          resolvedTitle = brollieStream.title;
          if (!resolvedTitle) {
            resolvedTitle = metadata.title;
          }
          if (!resolvedTitle) {
        ...
        args = await getYtDlpArgs(downloadUrl);

        if (resolvedTitle) {
          const oIndex = args.indexOf('-o');
          if (oIndex !== -1) {
            const sanitized = resolvedTitle.replace(/[/\\:*?"<>|]/g, '_');
            args[oIndex + 1] = `${sanitized}.%(ext)s`;
          }
        }
      } catch (error) {
      ...
      ytDlp.stderr.on('data', (data) => {
        ...
        if (this.logService) {
          this.logService.appendLines(hash, data.toString()).catch(() => {});
        }
      });
  ...
  async moveDownloadedFiles(_hash) {
    try {
      ...
      for (const file of activeFiles) {
        // Skip system files and dot files
        ...
        try {
          // Use copyFile + unlink instead of rename to work across different mount points
          await fs.copyFile(sourceFile, targetFile);
          await fs.unlink(sourceFile);
        } catch (error) {
          this.logger?.warn(`Failed to move file ${file}:`, error);
          // If copy succeeded but unlink failed, try to clean up the target
          try {
  ...
  async handleDownloadError(hash, url, error, _metadata = {}) {
    ...
    try {
      ...
      const job = await this.jobManager.getJob(hash);
      if (job && job.retryCount < this.jobManager.maxRetries) {
        // Clean up partial files before retry
        await this.cleanupPartialsDirectory();

        this.logger?.info(
          `Cleaned up partial files for job ${hash} before retry`
        );
      }
      ...
      if (retriedJob) {
        ...
        this.logger?.info(
          `Max retries (${this.jobManager.maxRetries}) reached for ${url}, moved to failed directory`
        );
        if (this.addNotification) {
          await this.addNotification(
            'error',
            `Download abandoned after ${this.jobManager.maxRetries} retries: ${url}`,
            { hash, url }
          );
        }
      }
  ...
  async cancelDownload(jobHash) {
    try {
      ...
      if (
        ...
        this.activeDownloadHash !== jobHash ||
        !activeProcess
      ) {
  ...
  /**
   * Clean up all files in the partials directory
   */
  async cleanupPartialsDirectory() {
    try {
      const activeFiles = await fs.readdir(this.downloadsActiveDir);
      ...
      for (const file of activeFiles) {
        ...
        try {
          ...
          this.logger?.info(`Cleaned up partial file: ${file}`);
        } catch (error) {
          this.logger?.warn(`Failed to clean up file ${file}:`, error);
        }
      }
    } catch (error) {
      this.logger?.error('Error cleaning up partials directory:', error);
    }
  }
  ...
  getStatus() {
    ...
    return {
      ...
      activeDownloadHash: this.activeDownloadHash,
