...
class QueueProcessor {
  ...
  parseProgressLine(line, hash) {
    ...
    if (fragmentProgressMatch) {
      ...
      if (this.broadcastChange && (now - lastBroadcast) > this.PROGRESS_BROADCAST_THROTTLE) {
        this.logger?.debug(`Broadcasting progress update for ${hash}: ${progress.percentage || 'unknown'}%`);
    ...
    if (destinationMatch) {
      ...
      const now = Date.now();
      const lastBroadcast = this.lastProgressBroadcast.get(hash) || 0;
      if (this.broadcastChange && (!hadFilename || (now - lastBroadcast) > this.PROGRESS_BROADCAST_THROTTLE)) {
        this.logger?.debug(`Broadcasting filename update for ${hash}: ${filename}`);
        this.broadcastChange();
        this.lastProgressBroadcast.set(hash, now);
      }
      return;
  ...
  async completeDownload(hash, url, _metadata = {}) {
    try {
      ...
      // Move job to finished state using JobManager
      await this.jobManager.moveJob(hash, JobState.FINISHED);
