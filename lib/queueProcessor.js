...
class QueueProcessor {
  constructor(options = {}) {
    ...
    this.baseDir = options.baseDir || join(__dirname, '..');
    ...
    this.downloadsActiveDir = join(this.baseDir, 'data', 'partials');
    this.downloadsFinishedDir = join(this.baseDir, 'downloads');
  ...
  parseProgressLine(line, hash) {
    ...
    if (fragmentProgressMatch) {
      ...
      if (
        ...
        this.logger?.debug(
          `Broadcasting progress update for ${hash}: ${
  ...
  processYtDlpOutput(chunk, hash, bufferState) {
    ...
    lines.forEach((line) => {
      if (line.trim()) {
        // Process progress updates directly
    ...
    return new Promise((resolve, reject) => {
      ...
      (async () => {
        for await (const chunk of ytDlp.stdout) {
          ...
          stdout += data;
          this.processYtDlpOutput(data, hash, bufferState);
        }
      })();
      ...
      ytDlp.status.then((status) => {
        if (status.code === 0) {
          this.logger?.info(`Download completed successfully: ${url}`);
          resolve({ stdout, stderr });
        } else {
          const error = new Error(`yt-dlp exited with code ${status.code}`);
          error.stdout = stdout;
          error.stderr = stderr;
  ...
  async moveDownloadedFiles(_hash) {
    try {
      const activeEntries = await Array.fromAsync(
        Deno.readDir(this.downloadsActiveDir),
      );
      ...
      for (const entry of activeEntries) {
        ...
        const sourceFile = join(this.downloadsActiveDir, entry.name);
        const targetFile = join(this.downloadsFinishedDir, entry.name);
        ...
        try {
          ...
          this.logger?.warn(`Failed to move file ${entry.name}:`, error);
  ...
  async addCompletionNotification(url, hash) {
    try {
      ...
      await this.ensureDirectoryExists(dirname(notificationsFile));

      let notifications = [];
      try {
        const data = await Deno.readTextFile(notificationsFile);
