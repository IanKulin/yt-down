...
class QueueProcessor {
  ...
  async startDownload(item) {
    const { hash, url, filePath, title, retryCount, timestamp, sortOrder } = item;
    ...
    try {
      ...
      this.logger?.info(`Started download: ${url} (hash: ${hash}, retry: ${retryCount})`);

      // Start the download process
      const downloadPromise = this.downloadVideo(hash, url, { title, retryCount, timestamp, sortOrder });
      this.activeDownloads.set(hash, downloadPromise);

      // Handle completion
      downloadPromise
        .then(async () => {
          await this.completeDownload(hash, url, activeFilePath, { title, retryCount, timestamp, sortOrder });
        })
        .catch(async (error) => {
          await this.handleDownloadError(hash, url, activeFilePath, error, { title, retryCount, timestamp, sortOrder });
        })
        .finally(() => {
  ...
  async completeDownload(hash, url, activeFilePath, _metadata = {}) {
    try {
      ...
      const progress = this.downloadProgress.get(hash);
      if (progress?.filename && !queueItem.title) {
        queueItem.title = progress.filename;
      }
      ...
      await fs.writeFile(finishedFilePath, JSON.stringify(queueItem, null, 2));
  ...
  async handleDownloadError(hash, url, activeFilePath, error, _metadata = {}) {
    ...
    try {
      ...
      if (queueItem.retryCount >= maxRetries) {
        this.logger?.info(`Max retries (${maxRetries}) reached for ${url}, giving up`);
        await fs.unlink(activeFilePath);
        return;
      }
      ...
      await fs.unlink(activeFilePath);
      this.logger?.info(`Moved failed download back to queue: ${url} (retry ${queueItem.retryCount}/${maxRetries})`);
    } catch (moveError) {
      this.logger?.error(
